...
# -*- coding: utf-8 -*-
"""
Pure-data per-language-pair cleaning threshold on `llm_judge_score`.
...
def plot_pair_candlestick(
    ...
    for fi, start in enumerate(range(0, len(pairs), per_fig)):
        ...
        ax.set_xlabel("llm_judge_score")
        ax.set_title(
            f"Diagnostic candlesticks — {selection}  ({start + 1}-{start + len(chunk)} / {len(pairs)})\n"
            "blue = llm_judge_score distribution (min · p05 · [p25–p75] · p50 · p95 · max), red ▼ = T"
        )
        ax.invert_yaxis()
