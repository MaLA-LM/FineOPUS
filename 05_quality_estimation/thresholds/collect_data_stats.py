...
# -*- coding: utf-8 -*-
"""
Per language-pair scan of qe_score distributions on the already-scored
FineOPUS-Filtered-Stage3-Scored parquet shards.
...
For each language pair:
  - Streams the 'qe_score' column (only) across all shards
...
Usage (single machine):
  python collect_data_stats.py \
      --scored_dir /scratch/.../FineOPUS-Filtered-Stage3-Scored \
...
def sample_pair_scores(
    ...
    """Stream qe_score across shards and return a float32 sample array.
    ...
    if total_rows <= sample_rows:
        ...
        for shard in shards:
            try:
                ...
                for batch in pf.iter_batches(columns=["qe_score"], batch_size=200_000):
                    arr = np.asarray(batch.column("qe_score"), dtype=np.float32)
    ...
    for shard, n_rows in zip(shards, row_counts):
        ...
        try:
            ...
            remaining = target
            total_read = 0
            for batch in pf.iter_batches(columns=["qe_score"], batch_size=500_000):
                if remaining <= 0:
                    break
                arr = np.asarray(batch.column("qe_score"), dtype=np.float32)
                total_read += arr.size
...
def main() -> None:
    ...
    parser.add_argument(
        "--scored_dir",
        default="/scratch/project_462001069/opus_qe/merged",
    )
    parser.add_argument(
        "--output",
