...
# -*- coding: utf-8 -*-
"""
Per language-pair scan of similarity_score distributions on the already-scored
...
For each language pair:
  - Streams the 'similarity_score' column (only) across all shards
...
def sample_pair_scores(
    ...
    """Stream similarity_score across shards and return a float32 sample array.
    ...
    if total_rows <= sample_rows:
        ...
        for shard in shards:
            try:
                ...
                for batch in pf.iter_batches(columns=["similarity_score"], batch_size=200_000):
                    arr = np.asarray(batch.column("similarity_score"), dtype=np.float32)
    ...
    for shard, n_rows in zip(shards, row_counts):
        ...
        try:
            ...
            remaining = target
            total_read = 0
            for batch in pf.iter_batches(columns=["similarity_score"], batch_size=500_000):
                if remaining <= 0:
                    break
                arr = np.asarray(batch.column("similarity_score"), dtype=np.float32)
                total_read += arr.size
