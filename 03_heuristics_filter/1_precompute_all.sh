...
OUT_DIR="/scratch/project_462000964/FineOPUS/intermediate/original_filter_precompute"
...
TASK_LIST_FILE="incomplete_folders_precompute_filter.txt"
...
if [ "$NUM_INCOMPLETE" -eq 0 ]; then
    ...
    sbatch --array=1-$NUM_JOBS \
           --export=ALL,DATA_DIR="$DATA_DIR",OUT_DIR="$OUT_DIR",TASK_LIST_FILE="$TASK_LIST_FILE",STATS_FILE="$STATS_FILE",CHUNK_SIZE="$CHUNK_SIZE" \
           precompute_filter_chunk.slurm
fi