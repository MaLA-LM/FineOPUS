...
from dataset.flores200.langcodes import flores200_langcodes
...
SPECIAL_ALIASES: dict[str, list[str]] = {
    ...
    # OPUS uses no parens for Norwegian variants; REMEDY keeps the
    # parenthesised form so FLORES exact-match still works.
...
def build_mapping_from_codes(
    language_codes: dict[str, str],
    model_languages: set[str],
) -> dict[str, list[object]]:
    """Shared implementation of the 4-stage fuzzy matcher.

    Takes an arbitrary ``{lang_code: display_name}`` dict so each dataset
    adapter can reuse the exact same matching logic without duplicating it.
    """
    mapping: dict[str, list[object]] = {}
    for language_code, display_name in language_codes.items():
        is_supported, matched_model_language = _match_language(
            display_name, model_languages
        )
...
def build_model_language_mapping(model_languages: set[str]) -> dict[str, list[object]]:
    """FLORES-200 wrapper kept for backwards compatibility.

    ``stand_alone_modules.patch_remedy`` imports this symbol directly, so its
    signature and import path must remain stable.
    """
    return build_mapping_from_codes(flores200_langcodes, model_languages)
